          ...
          {[1, 2].map(s => (
            <React.Fragment key={s}>
              <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-semibold transition-all
                ${step > s ? 'bg-emerald-500 text-white' : step === s ? 'bg-accent text-white' : 'bg-raised text-subtle'}`}>
                {step > s ? <CheckCircle className="w-5 h-5" /> : s}
              </div>
              {s < 2 && <div className={`w-12 h-0.5 rounded transition-all ${step > s ? 'bg-emerald-500' : 'bg-raised'}`} />}
            </React.Fragment>
          ))}
        </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={name} onChange={e => setName(e.target.value)} placeholder="Your name"
                  className="w-full bg-canvas border border-line rounded-lg px-4 py-2.5 text-sm text-white placeholder-dark-500 focus:border-accent outline-none" />
              </div>
              <div>
            ...
            {publicKey ? (
              <div className="space-y-4">
                <div>
                  ...
                  <div className="bg-canvas border border-line rounded-lg p-3 font-mono text-xs text-muted break-all max-h-32 overflow-y-auto select-all">
                    {publicKey}
                  </div>
                </div>
                <div className="flex flex-wrap gap-2">
                  ...
                  <button onClick={generateKey} disabled={loading}
                    className="px-3 py-2 bg-raised hover:bg-raised text-muted text-xs rounded-lg transition flex items-center gap-1.5 disabled:opacity-50">
                ...
                {testResult?.connected ? (
                  ...
                  <div className="text-center py-4 bg-red-500/10 border border-red-500/20 rounded-lg">
                    ...
                    {testResult.error && <pre className="bg-canvas rounded p-2 text-xs text-red-400/70 font-mono max-h-20 overflow-auto">{testResult.error}</pre>}
