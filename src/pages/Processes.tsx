...
export default function Processes() {
  ...
  const SortIcon = ({ col }: { col: SortKey }) => {
    ...
    return sortDir === 'desc' ? <ChevronDown className="w-3 h-3 text-accent" /> : <ChevronUp className="w-3 h-3 text-accent" />;
  };
    ...
    <div className="h-full flex flex-col animate-fade-in">
          ...
          <tbody>
            {sorted.map(proc => (
              <tr key={`${proc.pid}-${proc.command}`} className="border-b border-dark-700/30 hover:bg-dark-800/50 transition text-xs">
                ...
                <td className="px-3 py-1.5 text-right">
                  <span className={`font-mono ${proc.memory > 50 ? 'text-red-400' : proc.memory > 20 ? 'text-amber-400' : 'text-dark-300'}`}>
                    {proc.memory.toFixed(1)}
                  </span>
                </td>
                <td className="px-3 py-1.5 max-w-[200px] md:max-w-[400px]">
                  <span className="text-dark-200 font-mono text-[11px] truncate block">{proc.command}</span>
                </td>
      ...
      {killDialog && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setKillDialog(null)}>
          <div className="bg-dark-800 border border-dark-600 rounded-xl p-5 max-w-sm w-full animate-fade-in" onClick={e => e.stopPropagation()}>
